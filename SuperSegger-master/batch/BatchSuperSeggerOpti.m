...
function intProcessXY( dirname_xy, skip, nc, num_c, clean_flag, ...
    CONST, startEnd, crop_box, autoomni, num_xy)
...
if startEnd(1) <= 3 && startEnd(2) >=3 && ~exist( stamp_name, 'file' )
    parfor(i=1:num_t,floor(workers / num_xy)) % through all frames
%    for i = 1:num_t
...
function opstr = genOmniposeCommand(dirname_xy)
    ...
    opstr = ['python -m omnipose --dir ' diralign ' --save_png --dir_above --no_npy --in_folders  --omni --pretrained_model bact_phase_omni --cluster --mask_threshold 1 --flow_threshold 0 --diameter 30 --exclude_on_edges'];
